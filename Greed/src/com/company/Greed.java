...
public class Greed extends JPanel {
    ...
    int[][] grid;
    int playerRow, playerCol, score, highScore;
    double progress;
    ...
    void move(int d) {
        if (availableDirections[d]) {
            ...
            playerCol = c;
            playerRow = r;
            progress =  score*1.0 / (nCols * nRows) * 100.0;
    ...
    void drawScore(Graphics2D g) {
        g.setColor(Color.gray);
        g.setFont(getFont());
        String s = format("highscore   %s    score   %s   progress   %.2f %%", highScore, score,progress);
